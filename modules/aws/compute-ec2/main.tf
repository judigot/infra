...
variable "os" { type = string default = "linux" }
...
data "aws_ami" "windows" {
  ...
  most_recent = true
  owners      = ["amazon"]
  filter { name = "name" values = ["Windows_Server-2025-English-Full-Base-*"] }
}

resource "aws_key_pair" "this" { key_name = var.ssh_key_name public_key = var.ssh_public_key }
...
resource "aws_security_group" "this" {
  ...
  dynamic "ingress" {
    ...
    content { from_port = 22 to_port = 22 protocol = "tcp" cidr_blocks = var.ssh_allowed_cidrs }
  }
  dynamic "ingress" {
    for_each = var.os == "windows" && length(var.rdp_allowed_cidrs) > 0 ? [1] : []
    content { from_port = 3389 to_port = 3389 protocol = "tcp" cidr_blocks = var.rdp_allowed_cidrs }
  }
  dynamic "ingress" {
    for_each = toset(var.app_ports)
    content { from_port = ingress.value to_port = ingress.value protocol = "tcp" cidr_blocks = var.app_allowed_cidrs }
  }
  egress { from_port = 0 to_port = 0 protocol = "-1" cidr_blocks = ["0.0.0.0/0"] }
}

resource "aws_instance" "this" {
  ami = var.custom_ami != "" ? var.custom_ami : (var.os == "windows" ? data.aws_ami.windows[0].id : data.aws_ami.ubuntu[0].id)
  instance_type               = var.instance_type
  subnet_id                   = var.subnet_id
  vpc_security_group_ids      = [aws_security_group.this.id]
  key_name                    = aws_key_pair.this.key_name
  associate_public_ip_address = true
  root_block_device { volume_type = var.volume_type volume_size = var.disk_size delete_on_termination = true encrypted = true }
